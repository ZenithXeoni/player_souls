...
public class BanSoulMessage extends FriendlyByteBuf {
	public BanSoulMessage(boolean pressed, boolean released) {
	...
	public static void apply(MinecraftServer server, ServerPlayer entity, ServerGamePacketListenerImpl handler, FriendlyByteBuf buf, PacketSender responseSender) {
		...
		server.execute(() -> {
			...
			if (pressed) {
				...
				BanSoulOnKeyPressedProcedure.execute(com.google.common.collect.ImmutableMap.<String, Object>builder().put("world", world).put("x", x).put("y", y).put("z", z).put("entity", entity).build());
			}
		});
	}
}
